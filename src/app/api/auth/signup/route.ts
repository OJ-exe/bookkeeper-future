...
import { createUser } from "@/lib/server/authService";
import { createSession } from "@/lib/server/sessionService";
import { authSignupSchema, parseValidation } from "@/lib/server/validation";
import { badRequest } from "@/lib/server/response";

export async function POST(request: Request) {
  try {
    const body = await request.json();
    const parsed = parseValidation(authSignupSchema, body);
